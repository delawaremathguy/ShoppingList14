...
* (*December*) When running on iOS 15, the PurchasedItemsTabView and the LocationsTabView overrun the TabBar, apparently due to an Apple design change in tab bars.  By adding a `Divider()` view as the last element of the VStacks that define the PurchasedItemsTabView and the LocationsTabView, there is no longer an overrun (because the list is not the last view of the VStack).
